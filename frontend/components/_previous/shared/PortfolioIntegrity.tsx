...
function ComponentBar({ component }: { component: IntegrityComponent }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs text-gray-500 w-32 shrink-0 truncate">{component.label}</span>
      ...
      <span className={cn('text-xs font-mono tabular-nums w-8 text-right', color.text)}>{component.score}</span>
      <span className="text-xs text-gray-700 w-6 text-right">{Math.round(component.weight * 100)}%</span>
    </div>
  );
}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {entity.issues && entity.issues.length > 0 && (
            <p className="text-xs text-gray-500 mt-0.5 truncate">{entity.issues[0]}</p>
          )}
        </div>
          ...
          <div className="flex flex-col items-center">
            ...
            {integrityData.trend && integrityData.trend.length > 1 && (
              <div className="mt-3">
                ...
                <p className="text-xs text-gray-600 text-center mt-1">6-month trend</p>
              </div>
            )}
          </div>
                  ...
                  <div key={e.entityId} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-red-500/5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {e.issues && e.issues.length > 0 && (
                        <p className="text-xs text-gray-500 truncate">{e.issues.join(' · ')}</p>
                      )}
                    </div>
                  </div>
                ))}
