...
export function VerifiedCloseWorkflow({ sessionId }: { sessionId: string }) {
  ...
  const steps: WorkflowStep[] = useMemo(() => {
    ...
    const nonRoutineEntries = journalEntries.filter((e) => e.type !== 'recurring' && e.type !== 'template');
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-600">4-step cryptographically verified close pipeline</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-gray-500 tabular-nums">{completedCount}/{steps.length}</span>
        </div>
      </div>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center relative z-10" style={{ width: '25%' }}>
              ...
              <p className={cn('text-[9px] text-center', step.status === 'locked' ? 'text-gray-700' : 'text-gray-500')}>{step.subtitle}</p>
              <p className={cn('text-[9px] mt-1 text-center max-w-[120px]', step.status === 'locked' ? 'text-gray-700' : 'text-gray-500')}>{step.detail}</p>
