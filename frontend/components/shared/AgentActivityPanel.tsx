    ...
    <div className="border border-[#262C48] rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-gray-600 mt-0.5">{config.label} · {age}</p>
        </div>
      ...
      {expanded && (
        <div className="px-3 pb-3 border-t border-[#1e2235]">
          <pre className="text-[10px] text-gray-500 mt-2 font-mono whitespace-pre-wrap break-all bg-[#0d1017] rounded p-2 max-h-32 overflow-y-auto">
    ...
    <div className="bg-[#141829] border border-[#262C48] rounded-xl overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {memoryCount > 0 && (
            <span className="px-2 py-0.5 rounded bg-[#7C5CFC]/10 text-[#7C5CFC] text-[10px] font-semibold tabular-nums">{memoryCount}</span>
          )}
          ...
          <div className="flex items-center gap-1 mb-3 border-b border-[#1e2235] pb-2">
            {([
              ...
              >
                {t.label}
                {t.count > 0 && (
                  <span className="text-[9px] tabular-nums">{t.count}</span>
                )}
              </button>
            ))}
          </div>
          ...
          {tab === 'memory' && (
            <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
              ...
              {memoryEntries.length > 20 && (
                <p className="text-[10px] text-gray-600 text-center pt-1">+{memoryEntries.length - 20} more entries</p>
              )}
            </div>
          )}
                    ...
                    <div key={d.id} className="flex items-start gap-3 p-3 rounded-lg border border-[#262C48]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] text-gray-600 uppercase">{d.decisionType}</span>
                          <span className={cn('text-[9px] font-semibold px-1.5 py-0.5 rounded', confColor)}>
                            {Math.round(conf * 100)}%
                          </span>
                          {d.engineVersion && (
                            <span className="text-[9px] text-gray-700 font-mono">{d.engineVersion}</span>
                          )}
                        </div>
                      </div>
                    </div>
