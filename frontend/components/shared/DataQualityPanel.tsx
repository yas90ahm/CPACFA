    ...
    <div className={cn('flex items-start gap-3 p-3 rounded-lg border', config.border, config.bg)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {exception.metric != null && (
          <p className="text-[10px] text-gray-500 mt-0.5 font-mono">Metric: {exception.metric.toLocaleString()}</p>
        )}
        <div className="flex items-center gap-2 mt-1.5">
          <span className={cn('text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded', config.bg, config.color)}>
            {config.label}
          </span>
          <span className="text-[9px] text-gray-600 uppercase">{exception.status}</span>
        </div>
      </div>
      {!readOnly && exception.status === 'open' && (
        <div className="flex gap-1 shrink-0">
          <button
            ...
            className="p-1.5 rounded bg-gray-500/10 text-gray-400 hover:text-white text-[10px]"
            title="Acknowledge"
          >
          ...
          <button
            type="button"
            onClick={onResolve}
            className="p-1.5 rounded bg-emerald-500/10 text-emerald-400 hover:text-emerald-300 text-[10px]"
            title="Resolve"
          >
    ...
    <div className={cn(
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {criticalCount > 0 && (
            <span className="px-2 py-0.5 rounded bg-red-500/10 text-red-400 text-[10px] font-semibold tabular-nums">{criticalCount}</span>
          )}
          {warningCount > 0 && (
            <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 text-[10px] font-semibold tabular-nums">{warningCount}</span>
          )}
      ...
      {expanded && openExceptions.length > 0 && (
        <div className="px-4 pb-4 space-y-2">
          ...
          {openExceptions.length > 8 && (
            <p className="text-[10px] text-gray-600 text-center pt-1">+{openExceptions.length - 8} more issues</p>
          )}
        </div>
      )}
    </div>
