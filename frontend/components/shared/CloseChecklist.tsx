    ...
    <div className="bg-[#141829] border border-[#262C48] rounded-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-600">{stats.completed}/{stats.total} tasks complete</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {stats.blocking > 0 && (
              <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-400 text-[10px] font-medium">
                <AlertTriangle className="w-3 h-3" />
                {stats.blocking} blocking · ~{stats.estimatedDays}d to cert
              </span>
            )}
            {stats.overdue > 0 && (
              <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-500/10 text-red-400 text-[10px] font-medium">
                <Clock className="w-3 h-3" />
                {stats.overdue} overdue
              </span>
            )}
            {stats.blocking === 0 && (
              <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 text-[10px] font-medium">
      ...
      {sessionItems.length > 0 && (
        <div className="px-5 py-3 border-b border-[#1e2235]">
          <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Certification Controls</p>
            ...
            {sessionItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-[#1a1d2e] transition-colors">
                ...
                {item.required && item.status === 'pending' && (
                  <span className="text-[9px] font-semibold text-red-400 bg-red-500/10 px-1.5 py-0.5 rounded">REQUIRED</span>
                )}
                {!readOnly && item.status === 'pending' && (
                  <div className="flex gap-1">
                    <button
                      ...
                      className="text-[10px] text-emerald-400 hover:text-emerald-300 px-2 py-1 rounded bg-emerald-500/10"
                    >
                      Complete
                    </button>
                    {!item.required && (
                      <button
                        ...
                        className="text-[10px] text-gray-500 hover:text-gray-300 px-2 py-1 rounded bg-gray-500/10"
                      >
                        Skip
                      </button>
                    )}
                ...
                {item.completedBy && (
                  <span className="text-[10px] text-gray-600">{item.completedBy}</span>
                )}
              </div>
            ))}
          </div>
        ...
        return (
          <div key={category} className="border-b border-[#1e2235] last:border-b-0">
            ...
            >
              ...
              <span className="text-[10px] text-gray-600 tabular-nums">{groupComplete}/{groupTotal}</span>
                    ...
                    >
                      ...
                      {!readOnly && step.status === 'pending' && !isBlocked && (
                        <button
                          ...
                          className="text-[10px] text-[#7C5CFC] hover:text-white px-2 py-1 rounded bg-[#7C5CFC]/10"
                        >
                          Sign Off
                        </button>
                      )}
                      {step.signedOffBy && (
                        <span className="text-[9px] text-emerald-500">✓ {step.signedOffBy}</span>
                      )}
                    </div>
                  );
                })}
