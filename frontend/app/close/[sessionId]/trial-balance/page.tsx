      ...
      cell: (r: TrialBalanceRow) => (
        <span
          className={cn(
            ...
            r.mappingStatus === 'unmapped' && 'bg-status-red',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-4 py-2 px-3 rounded-input bg-surface border border-border">
        ...
        <span className={cn('font-mono text-sm', balanced ? 'text-status-green' : 'text-status-red')}>
          Difference: <MoneyCell value={totalDebits - totalCredits} showDollar />
        </span>
        <span className="text-text-secondary text-sm">Account Count: {filtered.length}</span>
        {unmappedCount > 0 ? (
          <span className="px-2 py-0.5 rounded text-xs bg-status-red-dim text-status-red">Unmapped: {unmappedCount}</span>
        ) : (
      ...
      <DataTable<TrialBalanceRow>
        ...
        renderExpanded={(r) => {
          const entries = data?.glEntriesByAccount?.[r.accountCode] ?? [];
          return (
            <div>
                ...
                <tbody>
                  {entries.map((e, i) => (
                    <tr key={i}>
                      ...
                      <td>{e.description}</td>
                      <td className="font-mono text-right"><MoneyCell value={e.debit} /></td>
                      <td className="font-mono text-right"><MoneyCell value={e.credit} /></td>
                      <td>{e.source}</td>
                    </tr>
                  ))}
                </tbody>
