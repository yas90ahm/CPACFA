    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>Track assets and compute depreciation for the period</p>
        </div>
        {!readOnly && (
          <div className="flex gap-2">
            ...
            <button onClick={() => runDepreciation.mutate()} disabled={runDepreciation.isPending} className="flex items-center gap-1.5 px-4 py-1.5 text-sm text-white rounded-md hover:bg-accent/90 disabled:opacity-50 font-medium" style={{ background: 'var(--interactive-primary)' }}>
      ...
      {!readOnly && showForm && (
        <div className="p-5 rounded-lg space-y-4" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface)' }}>
          <h3 className="font-medium" style={{ color: 'var(--text-primary)' }}>New Fixed Asset</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <input placeholder="Asset Number" value={form.assetNumber} onChange={(e) => setForm({ ...form, assetNumber: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }} />
            ...
            <input placeholder="Asset Type" value={form.assetType} onChange={(e) => setForm({ ...form, assetType: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }} />
            <input placeholder="Cost" type="number" value={form.cost} onChange={(e) => setForm({ ...form, cost: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }} />
            <input placeholder="Residual Value" type="number" value={form.residualValue} onChange={(e) => setForm({ ...form, residualValue: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }} />
            <input placeholder="Useful Life (years)" type="number" value={form.usefulLifeYears} onChange={(e) => setForm({ ...form, usefulLifeYears: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }} />
            <select value={form.method} onChange={(e) => setForm({ ...form, method: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }}>
              <option value="straight_line">Straight Line</option>
              <option value="declining_balance">Declining Balance</option>
            </select>
            <input type="date" placeholder="Start Date" value={form.depreciationStartDate} onChange={(e) => setForm({ ...form, depreciationStartDate: e.target.value })} className="rounded-md px-3 py-1.5 text-sm" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface-sunken)', color: 'var(--text-primary)' }} />
          </div>
          <div className="flex gap-2">
            <button onClick={handleCreate} disabled={createAsset.isPending || !form.assetNumber || !form.cost} className="px-4 py-1.5 text-sm text-white rounded-md font-medium disabled:opacity-50" style={{ background: 'var(--interactive-primary)' }}>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-lg" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface)' }}>
            <div className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--text-tertiary)' }}>Total Depreciation</div>
            <div className="text-lg font-medium" style={{ color: 'var(--text-primary)' }}>{fmtMoney(summary.totalDepreciation)}</div>
          </div>
          {Object.entries(summary.byType ?? {}).map(([type, amount]) => (
            <div key={type} className="p-4 rounded-lg" style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', background: 'var(--bg-surface)' }}>
              <div className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--text-tertiary)' }}>{type}</div>
      ...
      {!isLoading && (!assets || assets.length === 0) && !showForm ? (
        <div className="p-12 text-center" style={{ background: 'var(--bg-surface)', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border-default)', borderRadius: '0.5rem' }}>
          ...
          <p className="text-lg font-medium mb-2" style={{ color: 'var(--text-primary)' }}>No fixed assets yet</p>
        ...
        <DataTable<FixedAsset>
          ...
          columns={[
            ...
            { id: 'status', header: 'Status', cell: (r) => <span className={cn('px-2 py-0.5 rounded text-xs font-medium')} style={r.status === 'active' ? { background: 'var(--status-success-bg)', color: 'var(--status-success)' } : { background: 'var(--bg-surface-sunken)', color: 'var(--text-secondary)' }}>{r.status}</span> },
            { id: 'actions', header: '', cell: (r) => (
              !readOnly ? <button onClick={() => deleteAsset.mutate(r.id)} className="hover:opacity-70" style={{ color: 'var(--status-error)' }}><Trash2 className="w-4 h-4" /></button> : null
            )},
          ]}
        />
      )}
      ...
      {runs && runs.length > 0 && (
        <div>
          <h2 className="text-lg font-display mb-3" style={{ color: 'var(--text-primary)' }}>Depreciation Runs</h2>
