...
export async function runSupervisor(
  ...
  context?: {
    ...
    /** Uncommitted draft adjustments (Save for Later) for workspace hydration — not used in export or buildFinancialStatements. */
  ...
  const toolContext = {
    ...
    draftAdjustments: context?.draftAdjustments,
  };
  ...
  const draftBlock =
    context?.draftAdjustments?.length &&
    context.draftAdjustments.length > 0
      ? `\n\n[Save for Later — Uncommitted draft adjustments (${context.draftAdjustments.length}):\n${context.draftAdjustments
          .slice(0, 15)
          .map((a) =>
            a.kind === 'journal'
              ? `  Journal: ${(a as { accountName?: string }).accountName ?? '—'} Dr ${(a as { debit?: number }).debit ?? 0} Cr ${(a as { credit?: number }).credit ?? 0}`
              : a.kind === 'cpa_bridge'
                ? `  CPA Bridge: ${(a as { standard?: string }).standard ?? '—'}`
                : JSON.stringify(a).slice(0, 80)
          )
          .join('\n')}\nThese are not committed; only committed and balanced data can be exported. User may apply them via proposeTrialBalanceAdjustment + HITL or resolve-ingest.]`
      : '';
  const userContent = input.entries?.length
    ? `Current Ledger (first 40 rows — analyze for traps e.g. Ferrari, Lease, Plug):\n${ledgerTable}\n\n${input.message}${draftBlock}\n\n[Trial balance available in this session. Use buildFinancialStatements with sessionId and tenantId only (from context)—do not pass entries. Or use forensicRescan with entries if you need to re-validate.]`
    : `${input.message}${draftBlock}`;
