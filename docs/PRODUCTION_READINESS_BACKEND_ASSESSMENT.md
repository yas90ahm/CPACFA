# Production Readiness Assessment — Backend Only

**Application:** Accounting close software (trial balance → BS/P&L, HITL, close controls, jobs)  
**Scope:** Backend (Node/TS) only. Financial correctness, auditability, security, reliability.  
**Rule:** Evidence only (file paths + symbols). No assumptions.
...
### 1) Data integrity & validation — **3.5/5**
...
### 2) Audit trail completeness (hash chain coverage) — **3/5**
...
**Risks:** Not every financial mutation is guaranteed to call `recordMaterialEvent`/`recordOverride` (e.g. some close flows may not append); reconciliation signoff is covered. No automated test that every critical path writes to the ledger.
...
### 4) JE lifecycle controls (SoD, approvals, period lock) — **4/5**
...
**Risks:** Exceptions and waivers are in DB; no automated test that recon state transitions are enforced (e.g. cannot sign off draft).

**Remediation:** Add state-machine tests and ensure signoff is only allowed for appropriate status.

---

### 6) Close readiness gating — **4/5**
...
**Risks:** Readiness is only enforced on export when closeSessionId is provided; other entry points (e.g. direct PDF without closeSessionId) may skip readiness.
...
| File upload: type and size | `src/routes/trial-balance/ingest.ts` — multer fileFilter (CSV/XLSX), 10 MB; `src/routes/ingestion.ts` — 10 MB; `src/routes/close/close_journal_entries.ts` — 20 MB; `src/routes/vector_store.ts` — isPdfBuffer check |
...
**Risks:** Auth bypass paths still exist (disabled in prod); no rate limit per tenant/user; no CSP or security headers beyond helmet; file upload uses memory storage (DoS for large concurrent uploads); ingestion_dedup_store uses file `.dedup.enc` and INGESTION_DEDUP_KEY (production requires key).
...
### 8) Reliability (jobs, retries, idempotency) — **4/5**
...
**Risks:** Worker runs in-process (single point of failure; no separate worker process doc); no lock timeout (stale locked jobs not reclaimed); agentic_cleanup is placeholder.
...
| No OpenTelemetry/tracing | Grep: no trace_id, span, or OpenTelemetry in src |
| No Prometheus/metrics | No prom-client or metrics endpoint |
| Console.log in places | e.g. `src/server.ts` startup; `src/routes/trial-balance/ingest.ts` — console.log for file received |
| Request ID middleware | `src/middleware/requestId.js` (if present) — not confirmed in server.ts |
...
### 10) Performance / scalability — **2.5/5**
...
| DB indexes | jobs (idx_jobs_poll, idx_jobs_type, idx_jobs_created_at); period_locks, audit_ledger, etc. have indexes; no single “large ledger” index strategy doc |
...
### 12) Integration safety (post-back, idempotent exports) — **2.5/5**
...
- **2-week production hardening sprint (top 12 tasks):**
  ...
  4. Add request-id middleware and include trace_id/request_id in logger meta.
  5. Add /health and /ready endpoints (DB ping optional).
  6. Add job lock timeout or heartbeat and reclaim stale locked jobs.
  7. Enforce request body size limit (e.g. express.json({ limit: '2mb' })).
