...
router.post('/sync-trial-balance', async (req: Request, res: Response) => {
  try {
    ...
    // Staged ERP sync is intentionally fail-closed until its persistence and
    // promotion services are implemented. Direct sync remains opt-in.
    if (staged !== false) {
      res.status(501).json({
    ...
    // ── Explicit direct-write mode (staged: false) ──
