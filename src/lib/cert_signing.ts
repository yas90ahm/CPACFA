...
function autoGenerateKeys(): void {
  ...
  console.warn(
    '[CERT_SIGNING] Ed25519 keys auto-generated for development. These keys are ephemeral and will change on restart. Set CERT_SIGNING_PRIVATE_KEY and CERT_SIGNING_PUBLIC_KEY for persistent signatures.'
  );
}
...
function loadKeys(): void {
  ...
  if (!_privateKey || !_publicKey) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error(
        'FATAL: Ed25519 signing keys are required in production. Run npm run keygen to generate a keypair.'
      );
    }
    autoGenerateKeys();
  }
...
export function signArtifactHash(hashHex: string): SignResult {
  ...
  if (!_privateKey || !_publicKey) {
    throw new Error(
      'FATAL: Ed25519 signing keys required. Run npm run keygen to generate a keypair.'
    );
  }
