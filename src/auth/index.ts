...
if (process.env.NODE_ENV === 'production' && (!process.env.JWT_SECRET || process.env.JWT_SECRET === DEV_SECRET)) {
  throw new Error('JWT_SECRET must be set to a non-default value in production. Set JWT_SECRET in the environment.');
}
const JWT_SECRET = rawSecret;
// Production can override via env (e.g. 7d if policy allows).
