...
import { round2, from, plus, minus } from '../utils/decimal.js';
  ...
  for (const k of keys) {
    ...
    if (typeof v === 'string') {
      try {
        ...
        if (s === '' || s === '-') return 0;
        const d = from(s);
        if (d.isFinite()) return round2(d.toNumber());
      } catch {
        /* skip */
      }
    }
...
export function runBankPipeline(input: BankPipelineInput): BankPipelineResult {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (amount != null && debit === 0 && credit === 0) {
      if (amount >= 0) credit = round2(amount);
      else debit = round2(from(amount).abs().toNumber());
    }
    if (debit > 0) totalDebits = plus(totalDebits, debit);
    if (credit > 0) totalCredits = plus(totalCredits, credit);
    runningBalance = minus(plus(runningBalance, credit), debit);
