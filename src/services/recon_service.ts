...
export async function createReconRun(
  ...
  const id = randomUUID();
  return repo.insertReconRun(pool, id, closeSessionId, type, status);
}
...
export async function ingestReconItems(
  ...
  const run = await repo.getReconRunById(pool, reconRunId);
  if (!run) throw new ReconError('Recon run not found', 'NOT_FOUND');
  const created: ReconItem[] = [];
  for (const input of items) {
    const id = randomUUID();
    const item = await repo.insertReconItem(pool, id, reconRunId, input);
    created.push(item);
  }
  await repo.updateReconRunStatus(pool, reconRunId, 'in_progress');
  return created;
}
...
export async function proposeMatches(
  ...
  await repo.insertReconMatchGroup(pool, groupId, reconRunId, {
    status: 'proposed',
  ...
  const group = await repo.getReconMatchGroupById(pool, groupId);
  if (!group) throw new ReconError('Match group not found after insert', 'NOT_FOUND');
  return group;
}

/** Confirm a proposed match group. */
export async function confirmMatchGroup(pool: Pool, matchGroupId: string): Promise<ReconMatchGroup> {
  const group = await repo.getReconMatchGroupById(pool, matchGroupId);
  ...
  await repo.updateReconMatchGroupStatus(pool, matchGroupId, 'confirmed');
  const updated = await repo.getReconMatchGroupById(pool, matchGroupId);
  return updated!;
}

/** Reject a proposed match group. */
export async function rejectMatchGroup(pool: Pool, matchGroupId: string): Promise<ReconMatchGroup> {
  const group = await repo.getReconMatchGroupById(pool, matchGroupId);
  if (!group) throw new ReconError('Match group not found', 'NOT_FOUND');
  if (group.status !== 'proposed') throw new ReconError('Only proposed groups can be rejected', 'VALIDATION');
  await repo.updateReconMatchGroupStatus(pool, matchGroupId, 'rejected');
  const updated = await repo.getReconMatchGroupById(pool, matchGroupId);
  return updated!;
}
...
export async function markTimingDifference(
  ...
  if (!run) throw new ReconError('Recon run not found', 'NOT_FOUND');
  const id = randomUUID();
  return repo.insertReconException(pool, id, reconRunId, {
    reason,
    status: 'open',
    linkedIssueId,
  });
...
export async function signOffReconRun(
  ...
  const run = await repo.getReconRunById(pool, reconRunId);
  if (!run) throw new ReconError('Recon run not found', 'NOT_FOUND');
  const signoff = await repo.upsertReconSignoff(pool, reconRunId, signedBy, notes);
  await repo.updateReconRunStatus(pool, reconRunId, 'signed_off');
  if (tenantId) {
...
/** Get recon run by id. */
export async function getReconRun(pool: Pool, id: string): Promise<ReconRun | null> {
  return repo.getReconRunById(pool, id);
}
...
export async function listReconMatchGroupsByRunId(pool: Pool, reconRunId: string): Promise<ReconMatchGroup[]> {
  return repo.listReconMatchGroupsByRunId(pool, reconRunId);
}

/** Return recon_item ids that are not in any match group (confirmed or proposed). */
export async function getUnmatchedReconItemIds(pool: Pool, reconRunId: string): Promise<string[]> {
  const items = await repo.listReconItemsByRunId(pool, reconRunId);
  const matched = new Set<string>();
  const groups = await repo.listReconMatchGroupsByRunId(pool, reconRunId);
  for (const g of groups) {
    if (g.status === 'proposed' || g.status === 'confirmed') {
      const itemIds = await repo.listReconMatchGroupItemIds(pool, g.id);
...
export async function getUnmatchedReconItems(pool: Pool, reconRunId: string): Promise<ReconItem[]> {
  const items = await repo.listReconItemsByRunId(pool, reconRunId);
  const matched = new Set<string>();
  const groups = await repo.listReconMatchGroupsByRunId(pool, reconRunId);
  for (const g of groups) {
...
export async function emitIssuesForUnmatchedAboveMateriality(
  ...
  const run = await repo.getReconRunById(pool, opts.reconRunId);
  if (!run) throw new ReconError('Recon run not found', 'NOT_FOUND');
  ...
  const unmatched = await getUnmatchedReconItems(pool, opts.reconRunId);
